...
const ColorBlindTest = () => {
  ...
  const fetchPreviousResult = async () => {
    try {
      const response = await fetch(`https://backend-opticai.onrender.com/color-blindness/color-blindness-test/${userId}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (pythonResponse.ok) {
        ...
        const nodeResponse = await fetch("https://backend-opticai.onrender.com/color-blindness/color-blindness-test", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
